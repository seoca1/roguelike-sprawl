...
### 문제
전투 화면에서 스킬 이펙트(특히 hit flash) 잔상이 사라지지 않음.
- 원인: `_draw_vfx_overlay`의 hit flash가 sparse pattern(`(x+y)%3==0`)으로만 그리며, `console.print`가 foreground만 설정하고 background는 기존값 유지
- 결과: flash 종료 후에도 colored background가 afterimage로 남음

### 수정 내용
`engine/combat_view.py` `_draw_vfx_overlay` 시작 부분에 명시적 clear 추가:
```python
...
### 검증
- Headless isolated test: flash 활성 시 94셀 → 만료 후 0셀 ✅
- OLD behavior 확인: 만료 후에도 94셀 잔존 (afterimage bug) ✅
- `test_vfx_overlay_no_afterimage` regression test 추가
- pytest: 4154 passed ✅

### 영향
- `prototype/src/roguelike_sprawl/engine/combat_view.py`
- `prototype/tests/unit/test_combat.py` (regression test)
...
## [2026-07-26] cross-project | Phase α — Fiction ↔ roguelike_sprawl 양방향 link 통합

**Status**: Complete

### Changes

**Game (roguelike_sprawl) side (17 files modified)**:
- `tools/build_dashboard.py` — `_scan_dir()` returns 4-tuple incl. `game_mission_ids`; catalog entries include `game_mission_id` field; new top-level field `stories_with_mission_link` count
- `dashboard/data/library_stats.json` — regenerated: 83 stories now show `game_mission_id` (was: 0); `stories_with_mission_link: 83`
- `prototype/src/roguelike_sprawl/data/story_resolver.py` — new functions `_read_game_mission_ids_from_fiction()`, `validate_game_mission_id_links()`; cross-project validation
- `prototype/scripts/verify_story_links.py` — extended to report `game_mission_id` orphans + pass/fail summary
- `prototype/.github/workflows/cross-project-integrity.yml` — new "Fiction→Game mission link lint" CI step (informational, non-blocking for now)
- 12 dashboard data files regenerated by `build_dashboard.py`

**Fiction side (144 files modified, EN+KO 1:1 paired)**:
- 72 EN + 72 KO files: added `game_mission_id` field to frontmatter (after `stem:` or `derivative_date:`)
- Initial α-2 pass: 72 stems where stem == mission_id (1:1 mapping)
- Option 3 fix: 11 additional files where stem ≠ mission_id but mission.story.source references the stem
  - Examples: `case_jackout-30sec` → mission `first_jack`; `marly_louisiana-god` → mission `delivery_to_finn`; `flatline_again` → mission `flatline_call`; `loa_voodoo_contact` → mission `voodoo_loa_encounter`; `the_choice` → mission `final_choice`; etc.

### Cross-Project Integrity

| Metric | Before | After |
|---|---|---|
| Fiction stories with `game_mission_id` | 0 | **83** |
| Resolved (mission exists) | n/a | 83 |
| Orphan (mission missing) | n/a | 0 |
| `dashboard/data/library_stats.json` `stories_with_mission_link` | 0 | 83 |

### 19 Pre-existing Orphan Source Stems (out of α scope)

Mission `story.source` references these stems but no Fiction EN file exists:
- Bridge trilogy: `chevette-run`, `chevette_nightshift_run`, `bridge-construct`, `boone_tokyo_electronics_arc`, `mona_bridge_arc`, `pacific_empire_arc`, `idoru-wedding-protocol`, `g_boys_arc`, `w_anchor_arc`, `virtual_light_data_key_arc`, `wendell_suburban_arc`, `eurydice_arc`, `yanaka_family_power_arc`, `fukuoka_ridership_arc`, `viktor_orbit_arc`
- Blue Ant: `bigend-laney-lunch`, `coolhunter-laney-tokyo`, `kombinat-node-hack`, `cayce_footage_audit_run`
- Sprawl-related but no EN file: `tokyo-courier-run`, `sense_net_archive_intrusion`

These are pre-existing data integrity issues (missions referencing non-existent Fiction stems), out of scope for Phase α. Recommend: either write missing stories OR remove orphan source references in a follow-up.
...
Missions: 111
Available stories: 105
[...all 111 missions report OK...]

Cross-project: Fiction → roguelike_sprawl mission links
  Fiction stories with game_mission_id: 83
  Resolved (mission exists): 83
...
- `ruff check scripts/ src/roguelike_sprawl/data/`: All passed
- `pytest tests/unit/test_story_resolver.py`: 15 passed
- Game → Fiction link remains intact (wiki/world/*.md primary source references unchanged)

### 영향 파일

**Game**:
- `tools/build_dashboard.py` (+35 lines)
- `dashboard/data/library_stats.json` (+83 game_mission_id entries)
- 11 other dashboard data files (regenerated)
- `prototype/src/roguelike_sprawl/data/story_resolver.py` (+59 lines)
- `prototype/scripts/verify_story_links.py` (+24 lines)
- `prototype/.github/workflows/cross-project-integrity.yml` (+7 lines)

**Fiction**:
- 144 derivative short story files (72 EN + 72 KO) — frontmatter `game_mission_id` field added

### 후속 (권장)

1. **Pre-existing orphan source stems** (19건): 미션의 `story.source`가 존재하지 않는 Fiction stem 참조 — 별도 cleanup 작업 필요
2. **α-3 분석 결과 활용**: 32 미사용 Fiction stem 중 Tier 1 (post-merger) 16개 — Salvation Phase 미션에 연결 검토
